...
describe("Testing portugueseController", () => {
    ...
    describe("Testing updateWord method", () => {
        it("Should call the ptInjector updateWord method", async () => {
            await portugueseController.updateWord(
                "teste",
                {
                    newValue: "testes",
                },
        ...
        it("Should return string telling that word is not registered", async () => {
            const data = await portugueseController.updateWord(
                "teste",
                {
                    newValue: "testes",
                },
    ...
    describe("Testing queryWordByCategory", () => {
        it("Should call the queryWordByCategory method", async () => {
            ...
            await portugueseController.queryWordByCategory(categoryId);

            expect(ptInjector.queryWordByCategory).toBeCalledTimes(1);
            expect(ptInjector.queryWordByCategory).toBeCalledWith(categoryId);
        });
